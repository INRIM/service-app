...
handler = logging.handlers.TimedRotatingFileHandler(
    "log/run_task.log", when="W0", backupCount=10
)
...
class App:
    ...
    def get_headers(self):
        return {"Content-Type": "application/json", "apitoken": self.api_key}

    def get_tasks(self) -> dict:
        url = f"{self.default_url}/client/polling/calendar_tasks"
        with httpx.Client(timeout=None) as client:
            res = client.get(url=url, headers=self.get_headers())
            return res.json()
...
def eval_timestamp(tmsp):
    if tmsp > 0:
        ...
        return (
            tz.fromutc((epoch + timedelta(microseconds=tmsp)))
            .replace(tzinfo=tz)
            .strftime(config_system["server_datetime_mask"])
        )
    return False
...
class DbTask:
    ...
    def get_tasks(self) -> list:
        ...
        tasks = []
        fields = {"_id": 0}
        for cal_task in calendarcoll.find(
            {"deleted": 0, "active": True, "tipo": "task"}, fields
        ):
            ...
            if cal_task["action"] == "add":
                ...
                elif cal_task["stato"] == "progress" and not exists(
                    f"/etc/systemd/system/{task_name}.timer"
                ):
                    tasks.append(cal_task)
            else:
                tasks.append(cal_task)
            if cal_task in tasks:
                ...
                calendarcoll.update_one(
                    {"rec_name": task_name}, {"$set": cal_task}
                )
        ...
        for cal_task in calendarcoll.find(q, fields):
            ...
            if cal_task["action"] == "add":
                if cal_task["stato"] == "done":
                    tasks.append(cal_task)
        if len(tasks) > 0:
            run_task_logger.info(
                f"found {len(tasks)} completed useless tasks to remove"
            )
        return tasks
    ...
    def parse_date(self, vals):
        ...
        if isinstance(vals, str) and self.isodate_regex.match(vals):
            ...
            value_date = datetime.strptime(
                v, config_system["server_datetime_mask"]
            ).strftime(config_system["ui_datetime_mask"])
        return value_date
...
def is_active(task_name) -> bool:
    res = subprocess.run(
        ["systemctl", "status", task_name], cwd="/", stdout=subprocess.PIPE
    ).stdout.decode("utf-8")
...
def chk_remove_task(task):
    if exists(f"/etc/systemd/system/{task['rec_name']}.timer"):
        execute = [
            "/automations/modules/calendar-task/remove-task.sh",
            f"{task['rec_name']}",
        ]
        try:
            res = subprocess.run(
                execute,
                cwd="/automations/modules/calendar-task",
...
for task in tasks:
    if task["action"] == "add":
        ...
        execute = [
            "/automations/modules/main-scheduler/make-service.sh",
            f"{task['title']}",
            f"{task['rec_name']}",
            f"{task['calendar']}",
        ]
        try:
            res = subprocess.run(
                execute,
                cwd="/automations/modules/main-scheduler",
                stdout=subprocess.PIPE,
            ).stdout.decode("utf-8")
        ...
        if exists(
            f"/etc/systemd/system/{task['rec_name']}.timer"
        ) and is_active(f"{task['rec_name']}.timer"):
            execute = [
                "/automations/modules/calendar-task/pause-task.sh",
                f"{task['rec_name']}",
            ]
            try:
                res = subprocess.run(
            ...
            run_task_logger.info(
                f"Pause task {task['title']} -> File not exist"
            )
        ...
        if exists(f"/etc/systemd/system/{task['rec_name']}.timer"):
            ...
            run_task_logger.info(
                f"Remove task {task['title']} -> File not exist"
            )
    elif task["action"] == "resume":
        if exists(
            f"/etc/systemd/system/{task['rec_name']}.timer"
        ) and not is_active(f"{task['rec_name']}.timer"):
            execute = [
                "/automations/modules/calendar-task/resume-task.sh",
                f"{task['rec_name']}",
            ]
            try:
                res = subprocess.run(
            ...
            run_task_logger.info(
                f"Resume task {task['title']} -> File not exist"
            )
