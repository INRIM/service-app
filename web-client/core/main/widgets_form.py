...
SUBMIT_LABELS = {"new": "Salva", "update": "Aggiorna"}
...
class FormIoWidgetBase(FormIoWidget, PageWidget):
    ...
    def render_report_html(self):
        ...
        values = {"form": self.form_report_data}
        base = """{% set new_page  %}
<div style="display:block; clear:both; page-break-after:before;"></div>
{% endset %}"""
        report = f"{self.report}{base}"
        html_report_doc = self.render_str_template(report, values)
        template = self.theme_cfg.get_template("reports", "report_doc")
        values = {"document": html_report_doc}
        html_report = self.render_template(template, values)
        return html_report
    ...
    def handle_header_footer(self, options):
        ...
        if rheader == "1":
            ...
            values = {"logo": self.settings.get("logo_img_url")}
            base = self.render_template(template, values)
            file_report = f"/tmp/header_{str(uuid.uuid4())}.html"
            with open(file_report, "w") as f:
                f.write(base)
            options["header-html"] = file_report
            options["margin-top"] = self.settings.get(
        ...
        if rfooter == "1":
            ...
            values = {
                ...
                "report_footer_title": self.settings.get(
                    "report_footer_title", self.title
                ),
            ...
            options["margin-bottom"] = self.settings.get(
    ...
    def render_form(self):
        ...
        if self.request.query_params.get("miframe"):
            return self.render_paget(template, values)
        else:
