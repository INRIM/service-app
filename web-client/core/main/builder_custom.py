...
class CustomBuilder(Builder):
    ...
    def compute_form_data_table(self):
        ...
        for k, v in data_v.items():
            if any(
                x in k for x in ["data_value", "_surveyRow_", "_dataGridRow_"]
            ):
                to_pop.append(k)
        for x in to_pop:
            data_v.pop(x)
...
class CustomForm(Form):
    ...
    def _load_components(self, component):
        ...
        if (
            not component.survey
            and not component.multi_row
            and not component.tabs
            and component.component_items
        ):
