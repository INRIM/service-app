...
class ContentServiceBase(ContentService):
        ...
        if component.dataSrc == "custom":
            ...
            if memc and not editing and use_cahe:
                ...
                if component.dataSrc in ["resource", "form"]:
                    component.resources = (
                        await self.gateway.get_ext_submission(
                    ...
                    if (
                        ...
                        if res.get("status") and res.get("status") == "error":
                            ...
                            component.resources = res.get("content", {}).get(
                                "data", []
                            )[:]
                    else:
                        component.resources = (
                            await self.gateway.get_remote_data_select(
                                component.url,
    ...
    async def compute_datagrid_add_row(self, key, num_rows, data={}):
    ...
    async def print_form(self):
        ...
        pkit = pdfkit.PDFKit(
            report_html, "string", options=options, verbose=True
        )
    ...
    async def fast_search_eval(self, data, field) -> list:
        ...
        await self.eval_data_src_componentes(
            page.components_ext_data_src, data=data["data"]
        )
    ...
    async def form_post_complete_response(self, response_data, response):
        ...
        if "error" in response_data.get("status", ""):
            ...
            return await self.gateway.complete_json_response(
                {"link": url, "reload": True, "status": "ok"}
            )
        else:
    ...
    async def execute_task(self, task_name):
        ...
        return await self.update_tasks(
            task_name, caledar_data, status={"status": "done"}
        )
    ...
    async def get_filters_for_model(self, model):
        ...
        if not model == "component":
            server_response = await self.gateway.get_record(model, "")
