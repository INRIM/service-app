...
@process_api.post(
    "/start/{process_model}/{process_name}", tags=["process start instance"]
)
async def start_process(
    request: Request, process_model: str, process_name: str
):
...
@process_api.post(
    "/complete/{process_model}/{process_act_name}",
    tags=["process complete task"],
)
async def complete_task(
    request: Request, process_model: str, process_act_name: str
):
    gateway = Gateway.new(
        request=request, settings=get_settings(), templates=templates
    )
...
async def modal_action(request: Request):
    gateway = Gateway.new(
        request=request, settings=get_settings(), templates=templates
    )
    submitted_data = await gateway.load_post_request_data()
