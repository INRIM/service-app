...
async def client_data_table(
    ...
    data = ContentService.eval_table_processing(submitted_data)
    ...
    data_list = await ContentService.process_data_table(res_content.get("content").get("data"), submitted_data)
    resp = {
        "draw": data['draw'] + 1,
